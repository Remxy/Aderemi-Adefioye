...
install.packages("doc2vec", repos = "http://cran.us.r-project.org")
...
```
```{r}
API_key <- "2qLDuHN4LJMP5d5YK9fPzlCx7"
API_secret <- "x8o70TIssayk12WVsWrFan7gaeE57skDodZI7PBrBHY1E10feq"
Access_token <- "798702648-YOM2C7JtBWLixaGPlwqiSuavyXHkHQzg6rZTEqu7"
Access_token_secret <- "RkJkqJnZz1ac5aXD2P6OeE9k3a7m0bYeVsfWb4lm2nFDG"
```
```{r}
...
no_of_tweets
```
#Clustering By Similarity Terms

#For ease of Processing , there is need to remove uninteresting or infrequent words.

#Remove Sparse Terms from the Term-document matrix-86% Empty space------
```{r}
Bank_Tweet_SparseTM <- removeSparseTerms(Bank_tweet_dtm, sparse = 0.86)
Bank_Tweet_SparseTM
```
...
```{r}
Bank_tweet_dtm <- DocumentTermMatrix(Bank_tweet_corpus_clean)
Bank_tweet_dtm_matrix <- as.matrix(Bank_tweet_dtm)
Bank_tweet_dtm_matrix <- sort(colSums(Bank_tweet_dtm_matrix), decreasing = TRUE)
head(Bank_tweet_dtm_matrix)
```

```{r}
Bank_tweet_dtm_matrix.df <- as.data.frame(Bank_tweet_dtm_matrix)
```
#Scale the data
```{r}
Bank_tweet_dtm.scale <- scale(Bank_tweet_dtm_matrix.df, center = TRUE, scale = TRUE)
```
#create the distance matrix
```{r}
Bank_tweet.dist <- dist(Bank_tweet_dtm.scale, method = "euclidian", diag = FALSE, upper = FALSE, p = 5)
```
#Cluster the data
```{r}
Bank_tweet.dist.fit <- hclust(Bank_tweet.dist, method = "ward.D")
```
##Visualization
```{r}
library(stats)
plot(Bank_tweet.dist.fit, xlab = "Popular Words Distr", ylab = "Height", hang = -0.01, check = TRUE, axes = TRUE, frame.plot = TRUE, ann = TRUE, main = "Cluster - Top 5 Banks Tweet", col = "red")
dev.copy(jpeg, 'HeightDistr.jpeg')
dev.off()
```
#Visualize the result using k=3 Cluster
```{r}
plot(Bank_tweet.dist.fit, hang = -0.01)
groups <- cutree(Bank_tweet.dist.fit, k =3)
rect.hclust(Bank_tweet.dist.fit, k = 3, border = "red")
```
...
#Analyze score for tOP 5 Banks------okay
```{r}
